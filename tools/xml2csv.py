    ...
    for i in range(0, len(file_list)):
        ...
        if os.path.isfile(xml_path) and os.path.splitext(xml_path)[1] in xml_format:
            ...
            try:
                ...
                count = count + 1
                print("{} - [INFO] - Name: {}, time: {:.2f}s, CSV saved to {}".format(
                    time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), file_list[i], time_end - time_start,
                    csv_path))

            except:
                print("{} - [ERROR] - Name: {}. Save CSV failed, pass.".format(
                    time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), file_list[i]))
                continue

    print("\n{} - [INFO] - Done. {}/{} success. total: {:.2f}s, avg.: {:.2f}s\n".format(
        time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), count, count_all, time_elapse, time_elapse / count))
